...
def instances_based_condition(df,col1,val1,col2,val2):
    ...
    """
    instance = df[(df[col1] > 35) & (df[col2] == val2 )]
    return instance
...
def agg_values_ina_month(df,date_col,agg_col, agg):
    ...
    """
    month = pd.to_datetime(df[date_col]).dt.strftime('%m')
    return pd.pivot_table(df, index = month, values = agg_col, aggfunc = agg  )
...
def group_values(df,col1,agg1):
   ...
   Keyword arguments:
    ...
    """
    # agg(Mean=('returns', 'mean'), Sum=('returns', 'sum'))
...
agg_mean_temp = agg_values_ina_month(weather,'Date/Time','Temp (C)', np.mean)
print(agg_mean_temp)
